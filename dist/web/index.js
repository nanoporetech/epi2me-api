...
"use strict";function e(e){return e&&"object"===typeof e&&"default"in e?e.default:e}var t=require("lodash"),s=require("rxjs"),r=e(require("graphql-tag")),o=require("apollo-cache-inmemory"),i=e(require("apollo-client")),n=require("apollo-link"),a=require("apollo-link-http"),c=require("url"),l=require("@lifeomic/axios-fetch"),u=e(require("axios")),h=e(require("crypto")),p=require("tunnel"),d=e(require("os")),g=e(require("socket.io-client")),f="https://epi2me.nanoporetech.com",w={local:!1,url:f,user_agent:"EPI2ME API",region:"eu-west-1",sessionGrace:5,uploadTimeout:1200,downloadTimeout:1200,fileCheckInterval:5,downloadCheckInterval:3,stateCheckInterval:60,inFlightDelay:600,waitTimeSeconds:20,waitTokenError:30,transferPoolSize:3,downloadMode:"data+telemetry",filetype:[".fastq",".fq",".fastq.gz",".fq.gz"],signing:!0,sampleDirectory:"/data"};const m="\npage\npages\nhasNext\nhasPrevious\ntotalCount\n",y="\nidWorkflowInstance\nstartDate\nworkflowImage{\n  workflow\n  {\n    rev\n    name\n  }\n}\n";const k=function(){const e=(e,t)=>{e.headers||(e.headers={});let s=t;if(s||(s={}),!s.apikey||!s.apisecret)return;e.headers["X-EPI2ME-APIKEY"]=s.apikey,e.headers["X-EPI2ME-SIGNATUREDATE"]=(new Date).toISOString();const r=[Object.keys(e.headers).sort().filter(e=>e.match(/^x-epi2me/i)).map(t=>`${t}:${e.headers[t]}`).join("\n"),e.body].join("\n"),o=h.createHmac("sha1",s.apisecret).update(r).digest("hex");e.headers["X-EPI2ME-SIGNATUREV0"]=o};return{version:"3.0.1118",setHeaders:(s,r)=>{const{log:o}=t.merge({log:{debug:()=>{}}},r);let i=r;if(i||(i={}),s.headers=t.merge({Accept:"application/json","Content-Type":"application/json","X-EPI2ME-CLIENT":i.user_agent||"api","X-EPI2ME-VERSION":i.agent_version||k.version},s.headers,i.headers),"signing"in i&&!i.signing||e(s,i),i.proxy){const e=i.proxy.match(/https?:\/\/((\S+):(\S+)@)?(\S+):(\d+)/),t=e[2],r=e[3],n={host:e[4],port:e[5]};t&&r&&(n.proxyAuth=`${t}:${r}`),i.proxy.match(/^https/)?(o.debug("using HTTPS over HTTPS proxy",JSON.stringify(n)),s.httpsAgent=p.httpsOverHttps({proxy:n})):(o.debug("using HTTPS over HTTP proxy",JSON.stringify(n)),s.httpsAgent=p.httpsOverHttp({proxy:n})),s.proxy=!1}}}}(),$=l.buildAxiosFetch(u),_=(e,t)=>{const{apikey:s,apisecret:r}=t.headers.keys;return delete t.headers.keys,k.setHeaders(t,{apikey:s,apisecret:r,signing:!0}),$(e,t)},I=new i({link:new n.ApolloLink(e=>{const{apikey:t,apisecret:s,url:r}=e.getContext(),o=a.createHttpLink({uri:c.resolve(r,"/graphql"),fetch:_,headers:{keys:{apikey:t,apisecret:s}}});return n.execute(o,e)}),cache:new o.InMemoryCache});u.defaults.validateStatus=e=>e<=504;const S=function(){const e=(e,t)=>{e.headers||(e.headers={});let s=t;if(s||(s={}),!s.apikey)return;if(e.headers["X-EPI2ME-ApiKey"]=s.apikey,!s.apisecret)return;e.headers["X-EPI2ME-SignatureDate"]=(new Date).toISOString(),e.url.match(/^https:/)&&(e.url=e.url.replace(/:443/,"")),e.url.match(/^http:/)&&(e.url=e.url.replace(/:80/,""));const r=[e.url,Object.keys(e.headers).sort().filter(e=>e.match(/^x-epi2me/i)).map(t=>`${t}:${e.headers[t]}`).join("\n")].join("\n"),o=h.createHmac("sha1",s.apisecret).update(r).digest("hex");e.headers["X-EPI2ME-SignatureV0"]=o},s=async e=>{const t=e?e.data:null;if(!t)return Promise.reject(new Error("unexpected non-json response"));if(e&&e.status>=400){let s=`Network error ${e.status}`;return t.error&&(s=t.error),504===e.status&&(s="Please check your network connection and try again."),Promise.reject(new Error(s))}return t.error?Promise.reject(new Error(t.error)):Promise.resolve(t)};return{version:"3.0.1118",headers:(s,r)=>{const{log:o}=t.merge({log:{debug:()=>{}}},r);let i=r;if(i||(i={}),s.headers=t.merge({Accept:"application/json","Content-Type":"application/json","X-EPI2ME-Client":i.user_agent||"api","X-EPI2ME-Version":i.agent_version||S.version},s.headers,i.headers),"signing"in i&&!i.signing||e(s,i),i.proxy){const e=i.proxy.match(/https?:\/\/((\S+):(\S+)@)?(\S+):(\d+)/),t=e[2],r=e[3],n={host:e[4],port:e[5]};t&&r&&(n.proxyAuth=`${t}:${r}`),i.proxy.match(/^https/)?(o.debug("using HTTPS over HTTPS proxy",JSON.stringify(n)),s.httpsAgent=p.httpsOverHttps({proxy:n})):(o.debug("using HTTPS over HTTP proxy",JSON.stringify(n)),s.httpsAgent=p.httpsOverHttp({proxy:n})),s.proxy=!1}},head:async(e,s)=>{const{log:r}=t.merge({log:{debug:()=>{}}},s);let o,i=s.url,n=e;s.skip_url_mangle?o=n:(n=`/${n}`,i=i.replace(/\/+$/,""),n=n.replace(/\/+/g,"/"),o=i+n);const a={url:o,gzip:!0};let c;S.headers(a,s);try{if(r.debug(`HEAD ${a.url}`),c=await u.head(a.url,a),c&&c.status>=400){let e=`Network error ${c.status}`;return 504===c.status&&(e="Please check your network connection and try again."),Promise.reject(new Error(e))}}catch(l){return Promise.reject(l)}return Promise.resolve(c)},get:async(e,r)=>{const{log:o}=t.merge({log:{debug:()=>{}}},r);let i,n=r.url,a=e;r.skip_url_mangle?i=a:(a=`/${a}`,n=n.replace(/\/+$/,""),a=a.replace(/\/+/g,"/"),i=n+a);const c={url:i,gzip:!0};let l;S.headers(c,r);try{o.debug(`GET ${c.url}`),l=await u.get(c.url,c)}catch(h){return Promise.reject(h)}return s(l,r)},post:async(e,r,o)=>{const{log:i}=t.merge({log:{debug:()=>{}}},o);let n=o.url;n=n.replace(/\/+$/,"");const a={url:`${n}/${e.replace(/\/+/g,"/")}`,gzip:!0,data:r,headers:{}};if(o.legacy_form){const e=[],s=t.merge({json:JSON.stringify(r)},r);Object.keys(s).sort().forEach(t=>{e.push(`${t}=${escape(s[t])}`)}),a.data=e.join("&"),a.headers["Content-Type"]="application/x-www-form-urlencoded"}S.headers(a,o);const{data:c}=a;let l;delete a.data;try{i.debug(`POST ${a.url}`),l=await u.post(a.url,c,a)}catch(h){return Promise.reject(h)}return o.handler?o.handler(l):s(l,o)},put:async(e,r,o,i)=>{const{log:n}=t.merge({log:{debug:()=>{}}},i);let a=i.url;a=a.replace(/\/+$/,"");const c={url:`${a}/${e.replace(/\/+/g,"/")}/${r}`,gzip:!0,data:o,headers:{}};if(i.legacy_form){const e=[],s=t.merge({json:JSON.stringify(o)},o);Object.keys(s).sort().forEach(t=>{e.push(`${t}=${escape(s[t])}`)}),c.data=e.join("&"),c.headers["Content-Type"]="application/x-www-form-urlencoded"}S.headers(c,i);const{data:l}=c;let h;delete c.data;try{n.debug(`PUT ${c.url}`),h=await u.put(c.url,l,c)}catch(p){return Promise.reject(p)}return s(h,i)},convertResponseToObject(e){if("object"===typeof e)return e;try{return JSON.parse(e)}catch(t){throw new Error(`exception parsing chain JSON ${String(t)}`)}}}}();class b{constructor(e){b.prototype.__init.call(this),b.prototype.__init2.call(this),b.prototype.__init3.call(this),b.prototype.__init4.call(this),b.prototype.__init5.call(this),b.prototype.__init6.call(this),b.prototype.__init7.call(this),b.prototype.__init8.call(this),b.prototype.__init9.call(this),b.prototype.__init10.call(this),b.prototype.__init11.call(this),b.prototype.__init12.call(this),b.prototype.__init13.call(this),b.prototype.__init14.call(this),b.prototype.__init15.call(this),b.prototype.__init16.call(this),b.prototype.__init17.call(this),b.prototype.__init18.call(this),this.options=t.assign({agent_version:S.version,local:!1,url:f,user_agent:"EPI2ME API",signing:!0},e),this.options.url=this.options.url.replace(/:\/\//,"://graphql."),this.options.url=this.options.url.replace(/\/$/,""),this.log=this.options.log,this.client=I}__init(){this.createContext=e=>{const{apikey:s,apisecret:r,url:o}=this.options;return t.merge({apikey:s,apisecret:r,url:o},e)}}__init2(){this.query=e=>({context:t={},variables:s={},options:o={}}={})=>{const i=this.createContext(t);let n;return n="string"===typeof e?r`
        ${e}
      `:"function"===typeof e?r`
        ${e(m)}
  ...
  `)}__init16(){this.status=this.query(r`
    query status {
      status {
        portalVersion
        remoteAddr
  ...
  `)}__init17(){this.healthCheck=()=>S.get("/status",this.options)}__init18(){this.regions=this.query(r`
    query regions {
      regions {
        idRegion
        description
