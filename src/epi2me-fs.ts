...
export class EPI2ME_FS extends EPI2ME {
  ...
  async processMessage(message?: AWS.SQS.Message): Promise<void> {
    ...
    if (this.config.options.downloadMode === 'data+telemetry') {
      ...
      const settings = asOptDictionary(this.config.workflow?.settings);

      const outputFormat = settings?.output_format;
      if (isString(outputFormat)) {
        fetchSuffixes.push(...outputFormat.trim().split(/[\s,]+/)); // any extra extensions generated by the workflow
      } else if (isArrayOf(isString)(outputFormat)) {
        fetchSuffixes.push(...outputFormat); // any extra extensions generated by the workflow
      }

      try {
        const fetchPromises = fetchSuffixes
          .map((suffixIn) => {
            return suffixIn ? '.' + suffixIn.replace(/^\./, '') : suffixIn;
          }) // map "" => ""; fastq => .fastq; .fastq => .fastq
          .map((suffix) => {
            ...
            if (suffix !== '' && String(messageBody.path).match(extensionCheck)) {
              this.log.debug(
                `download.processMessage: ${message.MessageId} skipping download of extra suffix ${suffix}`,
              );
