...
"use strict";
var request, AWS, queue, fs, path, os, chokidar, mkdirp, proxy, _config;
    ...
    on: function (eventName, eventHandler) {
        ...
        if (eventName === "folderChange") {
            ...
            that._fsWatcher = chokidar.watch(_config.options.inputFolder, {
                ignoreInitial: true, // MC-1836 - Queue initial files using fs.readdir
                depth: 0
            })
                .on('add',    function onFSAdd(path) {
                    eventHandler('add', path.replace(/^[\w\W]*[\\\/]/, ''));
                })
                .on('unlink', function onFSUnlink(path) {
                    eventHandler('unlink', path.replace(/^[\w\W]*[\\\/]/, ''));
                });
