...
metrichor.prototype = {
	...
	if(!this.log) {
	    var l = function (str) { console.log("[" + (new Date()).toISOString() + "] " + str); };
	    this.log.info  = l;
	    this.log.warn  = l;
	    this.log.error = l;
	}
...
//                            that.log.info("discovered queue " + queueURL);
                        ...
                        function (err) {
                            that.log.error("error looking up queue. " + String(err));
    ...
    session: function (cb) {
        ...
        if (!that._stats.sts_expiration ||
            ...
            that.token(that._idWorkflowInstance, function (tokenError, token) {
                if (tokenError) {
                    that.log.error("failed to fetch session token: " + tokenError.error ? tokenError.error : tokenError);
...
//                that.log.info(JSON.stringify(token));
    ...
    autoStart: function (id, cb) {
        ...
        that.start_workflow(id, function (workflowError, instance) {
            if (workflowError) {
                that.log.error("failed to start workflow: " + (workflowError && workflowError.error ? workflowError.error : workflowError));
    ...
    autoConfigure: function (instance, autoStartCb) {
        ...
        fs.readdir(that.inputFolder, function (readdirErr, files) {
            if (readdirErr) {
                that.log.error("error reading folder " + String(readdirErr));
            }
    ...
    receiveMessages: function (receiveMessageError, receiveMessages, cb) {
        ...
        if (receiveMessageError) {
            that.log.error("error in receiveMessage " + String(receiveMessageError));
            return cb();
        }
            ...
            that.sessionedS3(function (sessionError, s3) {
                ...
                try {
                    s3.deleteObject({
                        ...
                        if (deleteObjectErr) {
                            that.log.error(String(deleteObjectErr) + " " + String(deleteObjectErr.stack)); // an error occurred
                        } else {
                    ...
                    try {
                        sqs.deleteMessage({
                            ...
                            if (deleteMessageError) {
                                that.log.error("error in deleteMessage " + String(deleteMessageError));
                            }
...
//        that.log.info("telemetry:" + JSON.stringify(messageBody.telemetry));
        ...
        if (!messageBody.path) {
            that.log.error("invalid message: " + JSON.stringify(messageBody));
            return;
        }
        ...
        if (that.downloadMode === "data+telemetry") {
            ...
            that.sessionedS3(function (sessionError, s3) {
                ...
                rs.on("error", function (readStreamError) {
                    that.log.error("error in readstream " + readStreamError); /* e.g. socket hangup */
                    try {
    ...
    uploadHandler: function (item, successCb) {
        ...
        that.sessionedS3(function (sessionError, s3) {
            ...
            rs.on("error", function (readStreamError) {
                ...
                that.log.error("error in readstream: " + readStreamError);
            ...
            rs.on("open", function () {
                ...
                try {
                    s3.upload(params, options, function (uploadStreamErr) {
                        ...
                        if (uploadStreamErr) {
                            that.log.error("uploadStreamError " + String(uploadStreamErr));
            ...
            sqs.getQueueUrl({ QueueName: queueName }, function (getQueueErr, getQueue) {
                if (getQueueErr) {
                    if (that.proxy && (String(getQueueErr)).match(/Unexpected close tag/)) {
                        that.log.error("error in getQueueUrl. Could be an aws-sdk/SSL/proxy compatibility issue");
                    }

                    that.log.error("uploader: could not getQueueUrl: " + getQueueErr);
...
//        that.log.info("sending message " + JSON.stringify(message));
        ...
        try {
            sqs.sendMessage({
                ...
                if (sendMessageError) {
                    that.log.error("error sending message " + String(sendMessageError));
                    that.enqueueUploadJob(item, 1);
...
//                that.log.info("message sent " + JSON.stringify(message));
                mkdirp(folderTo, function (mkdirException) {
                    ...
                    fs.rename(fileFrom, fileTo, function (renameException) {
                        if (renameException) {
                            that.log.error("renameException " + String(renameException));
                        }
        ...
        that.sessionedSQS(function (sessionError, sqs) {
            ...
            try {
                sqs.getQueueAttributes({
                    ...
                    if (attrErr) {
                        that.log.error("error in getQueueAttributes: " + String(attrErr));
                        return cb();
                    }
