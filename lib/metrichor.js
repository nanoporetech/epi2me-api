...
class metrichor {
    ...
    loadUploadFiles() {
        ...
        if (!this.inputBatchQueue) this.inputBatchQueue = queue(1);
        ...
        if (!this._dirScanInProgress && remaining === 0) {
            ...
            readdir(input, ignorables, (err, files) => {
                ...
                if (err) return this.log.error(`Could not check input folder for new files. fs.readdir error: ${err}`);
    ...
    enqueueUploadFiles(files) {
        if (this.config.options.filetype === '.fastq') {
            ...
            this.inputBatchQueue.defer(batch_complete => {
                ...
                let statQ = queue(1);
                this.log.info(`countFileReads: count FASTQs per batched file`);
                files.forEach(file => {
                ...
                statQ.awaitAll(() => {
                    uploadWorkerPool.awaitAll(() => {
                        setTimeout(() => {
                            this.log.info("Done uploading batch of size: " + files.length);
                            batch_complete();
            ...
            utils.chunk(files, this.config.options.uploadBatchSize)
                .forEach((chunk) => {
                    ...
                    this.inputBatchQueue.defer(batch_complete => {
                        ...
                        uploadWorkerPool.awaitAll(() => {
                            this.log.info("Done uploading batch of size: " + chunk.length);
                            setTimeout(() => {
                                batch_complete();
                                this.loadUploadFiles();
                            });
    ...
    uploadJob(file, completeCb) {
        ...
        this.uploadHandler(file, (errorMsg) => {
            setTimeout(() => completeCb()); // Release slot in upload queue
