...
"use strict";
var request, AWS, queue, fs, path, os, mkdirp, proxy, _config, underscore, Promise;
...
metrichor.prototype = {
    ...
    stats: function (key) {
        if (this._stats[key]) {
            ...
            if (key === "upload") {
                this._stats.upload.total = (this._stats.upload.enqueueCount || 0) + (this._fileStash &&  this._fileStash.length ? this._fileStash.length : 0);
            }
        }
    ...
    _statInputFiles: function (files, parallelism) {
        ...
        var that = this,
            fsStatQueue = queue(parallelism || 20); // run 20 fs.stat in parallel
    ...
    loadUploadFiles: function () {
        ...
        function unstashFiles() {
            underscore.chain(that._inputFiles)
                .filter(function (d) {
                    return !d.in_flight && !d.uploaded;
                })
                .first(_config.options.uploadQueueThreshold)
                .forEach(that.enqueueUploadJob.bind(that));
        }

        if (!that._dirScanInProgress && (remaining === 0 || that._fileStash.length < _config.options.uploadQueueThreshold)) {
            ...
            fs.readdir(_config.options.inputFolder, function (err, files) {
                ...
                var f = files.filter(function (fn) {
                    return fn.match(fileExp) && !that._inputFiles.hasOwnProperty(fn);
                });

                that.log.info('fs.stat all input files');
                that._statInputFiles(f)
                    .then(function (file_list) {
                        file_list.forEach(function (d) {
                            that._inputFiles[d.file] = d;
                        });
                        unstashFiles();
                    })
                    .catch(function (e) {
        ...
        } else if (remaining < _config.options.uploadQueueThreshold) {
    ...
    timedEnqueueUploadJob: function (item) {
        ...
        that.uploadWorkerPool.defer(function (completeCb) {
            ...
            if (item.in_flight || item.uploaded) {
                that._stats.upload.queueLength -= 1;
                that._stats.upload.enqueueCount = that._stats.upload.enqueueCount ? that._stats.upload.enqueueCount - 1 : 1;
                return completeCb();
            }
            ...
            try {
                that.uploadHandler(item.file, function (errorMsg) {
                    if (errorMsg) {
