...
metrichor.prototype = {
    ...
    downloadAvailable: function (cb) {
        var that = this,
            downloadWorkerPoolRemaining = that.downloadWorkerPool.remaining();

        if (!cb) {
            cb = function noop() { return undefined; };
        }

        if (downloadWorkerPoolRemaining >= _config.options.downloadPoolSize / 2) { /* downloadPool is fully utilised */
    ...
    loadUploadFiles: function () {
        ...
        if (!that._dirScanInProgress && that.uploadWorkerPool.remaining() === 0) {
        ...
        if (that.uploadWorkerPool.remaining() < 1000) {
            that._fileStash
                .splice(0, _config.options.uploadQueueThreshold)
                .forEach(function unstashFiles(fn) {
                    return that.enqueueUploadJob(fn);
    ...
    timedEnqueueUploadJob: function (item, requeue) {
        var that = this;
