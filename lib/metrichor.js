...
class metrichor {
    ...
    autoConfigure(instance, autoStartCb) {
        ...
        this.log.debug("delaying inputBatchQueue slot");
        this.inputBatchQueue.defer((cb) => {
            setTimeout(() => {
                this.log.debug("freeing inputBatchQueue slot");
                cb();
    ...
    loadUploadFiles() {
        ...
        this.log.debug(`${remaining} batches in the inputBatchQueue`);
        ...
        if (!this._dirScanInProgress && remaining === 0) {
            ...
            readdir(input, ignorables, (err, files) => {
                ...
                let fileSizeQ = queue(2);
                files = files.map(file => {
                    let parsed     = path.parse(file)
                    let fileObject = { name: parsed.base, path: file }
                    ...
                    fileSizeQ.defer(done => {
                        utils.getFileSize(file)
                            ...
                            .catch(err => {
                                this.log.error("fileSizeQ, getFileSize", err);
                                done();
                            });
                    });
                ...
                this.log.debug(`items in input folder: ${files.length}\tnew ${this.config.options.filetype} files: ${files.length}`);

                fileSizeQ.awaitAll(() => {
                    this._dirScanInProgress = false;
                    this.log.info(`enqueuing ${files.length} files for upload`);
                    // Get the file size of all files before progressing to the next step
    ...
    enqueueUploadFiles(files) {
        if (this.config.options.filetype === '.fastq') {
            this.inputBatchQueue.defer(batch_complete => {
                ...
                let statQ = queue(1);
                this.log.debug(`countFileReads: count FASTQs per batched file`);
                files.forEach(file => {
    ...
    uploadHandler(item, completeCb) {
        ...
        rs.on("open", () => {
            ...
            if (item.size) {
                params["Content-Length"] = item.size;
            }
                ...
                try {
                    ...
                    readStream
                        .on('close', () => {
                            ...
                            statFile();
                            this.log.debug("marked " + fileFrom + " as done");
