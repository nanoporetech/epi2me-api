...
class metrichor {
    ...
    _list(entity, cb) {
        return utils._get(entity, this.config.options, function (e, json) {
            if (cb) {
                cb(e, json[entity + "s"]);
            }
        });
    ...
    fetchInstanceToken(queueCb) {
        ...
        that.token(this.config.instance.id_workflow_instance, (tokenError, token) => {
            if (tokenError) {
            ...
            this._stats.sts_expiration.setMinutes(that._stats.sts_expiration.getMinutes() - this.config.options.sessionGrace); // refresh token x mins before it expires
    ...
    sessionedS3() {
        this.session();
        return new AWS.S3({useAccelerateEndpoint: this.config.options.awsAcceleration === "on", StorageClass: 'REDUCED_REDUNDANCY'});
    }
    ...
    autoConfigure(instance, autoStartCb) {
        ...
        telemetryLogPath = path.join(this.config.options.outputFolder, fileName);
        ...
        // sqs event handler
        this._fileCheckInterval = setInterval(that.loadUploadFiles.bind(this), this.config.options.fileCheckInterval * 1000);

        this.inputBatchQueue = queue(1);
        this.log.info("delaying inputBatchQueue slot");
        this.inputBatchQueue.defer(function (cb) {
            setTimeout(() => {
        ...
        this._stateCheckInterval = setInterval(() => {
            this.workflow_instance(this.config.instance.id_workflow_instance, function _stateCheckCB(instanceError, instance) {
                if (instanceError) {
    ...
    loadUploadFiles() {
        ...
        if (!this._dirScanInProgress && remaining === 0) {
            ...
            fs.readdir(this.config.options.inputFolder, (err, files) => {
                ...
                let new_files = files.filter((fn) => fn.match(fileExp) && this._uploadedFiles.indexOf(fn) === -1);

                this.log.info(`items in input folder: ${files.length}\tnew ${this.config.options.inputFormat} files: ${new_files.length}`);

                utils.chunk(new_files, this.config.options.uploadBatchSize)
                    .forEach((chunk) => {
                        this._stats.upload.enqueued += chunk.length;

                        this.inputBatchQueue.defer(function (batch_complete) {

                            let uploadWorkerPool = queue(this.config.options.uploadPoolSize);

                            chunk.forEach((item) => uploadWorkerPool.defer(that.uploadJob.bind(this, item)));
    ...
    uploadJob(item, completeCb) {
        // Initiate file upload to S3
        this.uploadHandler(item, (errorMsg) => {
            completeCb(); // Release slot in upload queue
            this._stats.upload.enqueued = that._stats.upload.enqueued - 1;
            if (errorMsg) {
                ...
                this._stats.upload.queueLength = this._stats.upload.queueLength ? this._stats.upload.queueLength - 1 : 0;
                this._stats.upload.success     = this._stats.upload.success     ? this._stats.upload.success     + 1 : 1;
            }
        });
    }
    ...
    processMessage(message, completeCb) {
        let outputFile, messageBody, fn, folder, match, exit_status, s3;
        ...
        if (this.config.options.downloadMode === "data+telemetry") {
            ...
            this._stats.download.success = this._stats.download.success ? this._stats.download.success + 1 : 1; // hmm. not exactly "download", these
    ...
    _initiateDownloadStream(s3, messageBody, message, outputFile, completeCb) {
        ...
        file.on("finish", () => {
            if (!file._networkStreamError) {
                ...
                this._stats.download.success = this._stats.download.success ? this._stats.download.success + 1 : 1;
    ...
    uploadHandler(item, completeCb) {
        ...
        let s3 = this.sessionedS3(),
            rs,
            fileId   = path.join(this.config.options.inputFolder, item),
            objectId = this.config.instance.bucketFolder + "/" + (this.config.instance.inputQueueName ? this.config.instance.inputQueueName + "/" : "") + item,
            timeoutHandle,
            completed = false;
        ...
        timeoutHandle = setTimeout(() => {
            ...
            done("this.uploadWorkerPool timeoutHandle. Clearing queue slot for file: " + item);
    ...
    uploadComplete(objectId, item, successCb) {
        let sqs = this.sessionedSQS();
        this.log.info("uploaded " + item + " to " + objectId);
    ...
    sendMessage(sqs, objectId, item, successCb) {
        ...
        if (this.config.options.agent_address) message.agent_address = this.config.options.agent_address;
    ...
    _moveUploadedFile(fileName, successCb) {
        ...
        let folderTo, fileTo, fileFrom, streamErrorFlag, readStream, writeStream, renameComplete;

        if (this.config.options.uploadedFolder && this.config.options.uploadedFolder !== '+uploaded') {
            folderTo =  this.config.options.uploadedFolder;
        } else {
            folderTo = path.join(this.config.options.inputFolder, "uploaded");
        }
        fileFrom = path.join(this.config.options.inputFolder, fileName);
