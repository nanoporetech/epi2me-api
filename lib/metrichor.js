    ...
    receiveMessages: function (receiveMessageError, receiveMessages, cb) {
        ...
        receiveMessages.Messages.forEach(function (message) {
            that.downloadWorkerPool.defer(function (queueCb) {
                ...
                timeoutHandle = setTimeout(done, (100 + _config.options.downloadTimeout) * 1000);
...
//                message.inputFolder = that.runFolder; // MC-960 folder aggregation messages
    ...
    _moveUploadedFile: function (fileName, successCb) {
        ...
        var that = this, folderTo, fileTo, fileFrom, streamErrorFlag;
        ...
        function onError(err) {
            if (err && !streamErrorFlag) {
                ...
                that._uploadedFiles[fileName] = streamErrorFlag = true; // flag as uploaded

                try {
                    statFile(fileFrom);
                    fs.unlink(fileTo, function fsUnlinkCallback(err) {
                        // clean up fileTo on error event.
                        if (err) {
                            that.log.error("failed to remove uploaded file " + fileTo + " : " + err);
                        }
                        successCb();
                    });
                } catch (err) {
                    that.log.error("error removing uploaded target file " + fileTo + " : " + err);
                    successCb();
                }
        ...
        mkdirp(folderTo, function (mkdirException) {
            if (mkdirException && !String(mkdirException).match(/EEXIST/)) {
                ...
                try {
                    ...
                    readStream
                        .on('close', function () {
                            fs.unlink(fileFrom, function fsUnlinkCallback(err) {
                                if (err) {
                            ...
                            successCb(); // close the queue job // SUCCESS
                        })
