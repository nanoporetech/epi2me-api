...
function metrichor(opt_string) {
    ...
    _config = {
        options: {
            ...
            uploadQueueLimit       : opts.uploadQueueLimit || 500,          // Number of files loaded into the queue at once
...
//                that.log.info(JSON.stringify(token));
    ...
    loadUploadFiles: function () {
        ...
        if (that._dirScanInProgress) {
            return;
        }

        if (!that._dirScanInProgress && that._fileStash.length < _config.options.uploadQueueThreshold) {
            ...
            fs.readdir(_config.options.inputFolder, function (err, files) {
                ...
                that._fileStash = files.filter(function (fn) {
                    if (fn.match(fileExp) && !that._seenFiles[fn]) {
                        return that._seenFiles[fn] = true;
                    }
                });
            });
        }
        that._fileStash.splice(0, 500).forEach(that.enqueueUploadJob.bind(that));
    },
                ...
                that.uploadHandler(item, function (result) {
                    if (isNaN(result)) {
                        ...
                        if (that._seenFiles[item]) {
                            delete that._seenFiles[item].state;
                        }
