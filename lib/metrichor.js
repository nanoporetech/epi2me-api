...
"use strict";
var request, jqWrap, AWS, queue, fs, path, os, mkdirp;
...
function metrichor(opt_string) {
    ...
    /* upload settings */
    this._inputFolder     = opts.inputFolder;
    this.inputQueueName   = opts.inputQueueName;
    this.chain            = opts.chain;
    this.uploadPoolSize   = 10;

    /* download settings */
    this._outputFolder    = opts.outputFolder;
...
metrichor.prototype = {
    ...
    on: function (eventName, eventHandler) {
        ...
        if (eventName === "folderChange") {
            ...
            that._fsWatcher = fs.watch(that._inputFolder, function (event, filename) {
        ...
        if (eventName === "downloadAvailable") {
            ...
            workChecker = function (len) {
                that.log("download checker len=" + len);
                if (len) {
    ...
    autoConfigure: function () {
        ...
        fs.readdir(that._inputFolder, function (readdirErr, files) {
            if (readdirErr) {
        ...
        mkdirp.sync(that._outputFolder);
        telemetryLogPath = path.join(that._outputFolder, "telemetry.log");
            ...
            that.sessionedS3(function (s3) {
                try {
                    s3.deleteObject({
                        ...
                        if (deleteObjectErr) {
                            that.log(deleteObjectErr, deleteObjectErr.stack); // an error occurred
                        } else {
                    ...
                    try {
                        sqs.deleteMessage({
                            ...
                            if (deleteMessageError) {
                                that.log("error in deleteMessage", deleteMessageError);
                            }
                            that.log("deleteMessage success");
                        });

                    } catch (deleteMessageErr) {
                        that.log("deleteMessage exception: " + JSON.stringify(deleteMessageErr));
                    }
                },
                function (reason) {
                    that._stats.download.failure[reason] = that._stats.download.failure[reason] ? that._stats.download.failure[reason] + 1 : 1;
...
//        that.log("telemetry:" + JSON.stringify(messageBody.telemetry));
        ...
        if (that.downloadMode === "data+telemetry") {
            ...
            that.sessionedS3(function (s3) {
                ...
                file.on("close", function (writeStreamError) {
                    ...
                    that.log("download success");

                    if (that.requestSuccess) {
                        that.requestSuccess(messageBody.path, outputFile);
                    }
    ...
    uploadHandler: function (item, successCb) {
        ...
        that.sessionedS3(function (s3) {
            var rs,
                fileId   = path.join(that._inputFolder, item),
...
//                message.inputFolder = that.runFolder; // MC-960 folder aggregation messages
        ...
        try {
            sqs.sendMessage({
                ...
                var fileFrom = path.join(that._inputFolder, item),
                    folderTo = path.join(that._inputFolder, "uploaded"),
