...
class metrichor {
    ...
    loadUploadFiles() {
        ...
        if (!this._dirScanInProgress && remaining === 0) {
            ...
            // console.log(this.uploadTo)
            // return
            let ignorables = [`${this.uploadTo}`, "/downloads", '/uploaded', nonFast5]
            // let ignorables = [nonFast5]
    ...
    processMessage(message, completeCb) {
        ...
        // console.log('save', '/downloads', outputFile)
    ...
    _initiateDownloadStream(s3, messageBody, message, outputFile, completeCb) {
        ...
        file.on("finish", () => {
            if (!file._networkStreamError) {
                ...
                if (!this._stats.download.success) this._stats.download.success = 0
                this._stats.download.success += 1
