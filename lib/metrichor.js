...
"use strict";
var request, AWS, queue, fs, chokidar, path, os, mkdirp, proxy, _config;
...
metrichor.prototype = {
    ...
    on: function (eventName, eventHandler) {
        ...
        if (eventName === "folderChange") {
            ...
            // MC-1770 - _config.options.outputFolder???
            that._fsWatcher = chokidar.watch(_config.options.inputFolder, {
                // ignored: new RegExp("^((?!\\.fast5).)*$", 'i'), // Ignore all files that does not contain .fast5
               // persistent: true,
                depth: 0
            })
                .on('add',    function onFSAdd(path) { eventHandler('add', path); })
                .on('change', function onFSChange(path) { eventHandler('change', path); })
                .on('unlink', function onFSUnlink(path) { eventHandler('unlink', path); });
...
//                that.log.info(JSON.stringify(token));
    ...
    autoConfigure: function (instance, autoStartCb) {
        ...
        that.on("folderChange", function (event, item) {
            if (item && item.match(/\.fast5$/)) {
                if (event === "change" || event === "add") {
                    that.enqueueUploadJob(item); // file added or renamed from .fast5.tmp
                    that._stats.upload.queueLength += 1;
                } else if (event === "unlink") { // file moved
                    that._stats.upload.queueLength -= 1;
                }
...
//        that.log.info("telemetry:" + JSON.stringify(messageBody.telemetry));
        ...
        match      = messageBody.path.replace(/^[\w\W]*[\\\/]/, '');
        fn         = match ? match : "";
        ...
        if (_config.options.downloadMode === "data+telemetry") {
            ...
            that.sessionedS3(function (sessionError, s3) {
                ...
                file.on("close", function (writeStreamError) {
                    ...
                    // MC-404/MC-551 support for arbitrary work on downloaded analysis file
                    if (_config.options.dataCb) {
                        _config.options.dataCb(outputFile, messageBody.telemetry);
                    }
    ...
    uploadHandler: function (item, successCb) {
        ...
        that.sessionedS3(function (sessionError, s3) {
            ...
            try {
                ...
                if (successCb) {
                    successCb("readstream exception");
                }
                return; // close the queue job
            }
        ...
        try {
            sqs.sendMessage({
                ...
                mkdirp(folderTo, function (mkdirException) {
                    ...
                    fs.rename(item, fileTo, function (renameException) {
                        if (renameException) {
                            that.log.warn("renameException " + String(renameException));
                        }

                        that.log.info("marked " + fileName + " as done");
                        if (successCb) {
                            successCb(1);
