...
class metrichor {
    constructor(opt_string) {
        ...
        _config.options = _.defaults(opts, defaults);
    ...
    sessionedS3() {
        this.session();
        return new AWS.S3({useAccelerateEndpoint: _config.options.awsAcceleration === "on", StorageClass: 'REDUCED_REDUNDANCY'});
    }
    ...
    autoJoin(id, cb) {
        _config.instance.id_workflow_instance = id;
    ...
    autoConfigure(instance, autoStartCb) {
        ...
        _config.instance.awssettings.region   = instance.region || _config.options.region;
        ...
        if (!_config.options.inputFolder) throw new Error("must set inputFolder");
        if (!_config.options.outputFolder) throw new Error("must set outputFolder");
        if (!_config.instance.bucketFolder) throw new Error("bucketFolder must be set");
        if (!_config.instance.inputQueueName) throw new Error("inputQueueName must be set");
        if (!_config.instance.outputQueueName) throw new Error("outputQueueName must be set");
        ...
        if (autoStartCb) autoStartCb(null, _config.instance);
        ...
        that._stateCheckInterval = setInterval(function () {
            that.workflow_instance(_config.instance.id_workflow_instance, function _stateCheckCB(instanceError, instance) {
                if (instanceError) {
                    ...
                    if (instance.state === "stopped") {
                        that.log.warn("instance was stopped remotely at " + instance.stop_date + ". shutting down the workflow.");
    ...
    processMessage(message, completeCb) {
        var outputFile, messageBody, fn, folder, match, exit_status, s3,
            that = this;
                ...
                try {
                    fs.stat(outputFile, function fsStatCallback(err, stats) {
                        if (err) {
                            ...
                            if (exit_status && _config.options.dataCb) {
    ...
    workflow_config(id, cb) {
        return utils._get('workflow/' + id, _config.options, cb);
    }
}
