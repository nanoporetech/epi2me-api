...
metrichor.prototype = {
    ...
    session: function (sessionCb) {
        ...
        if (!sessionCb) {
            sessionCb = function sessionNoOp() { return undefined; };
        }
        ...
        if (that.sessionQueue.remaining()) {
            /* Throttle to n=1: bail out if there's already a job queued */
            return sessionCb();
        }

        /* Hope the queued request comes back in under _config.options.sessionGrace time */
        that.sessionQueue.defer(function (queueCb) {
            that.fetchInstanceToken(function (arg) {
                queueCb();             /* free up the sessionQueue slot   */
                return sessionCb(arg); /* does the order of these matter? */
            });
        });
    },

    fetchInstanceToken: function (sessionCb) {
        var that = this;
        ...
        that.token(_config.instance.id_workflow_instance, function (tokenError, token) {
            if (tokenError) {
                ...
                return sessionCb("failed to fetch instance token");
            }
        ...
        /* if existing instance is invalid. sessionCb is called on success+failure in that.token callback, above */
    },
