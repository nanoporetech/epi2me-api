...
metrichor.prototype = {
    ...
    session: function (sessionCb) {
        ...
        that.sessionQueue.defer(function (cb) {
            that.fetchInstanceToken(cb, sessionCb);
        });
    },

    fetchInstanceToken: function (queueCb, sessionCb) {
        ...
        that.token(_config.instance.id_workflow_instance, function (tokenError, token) {
            ...
            that._stats.sts_expiration.setMinutes(that._stats.sts_expiration.getMinutes() - 5); // refresh token 2 mins before it expires
