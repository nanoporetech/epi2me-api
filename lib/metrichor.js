...
const readdir = require('recursive-readdir') // handle batching. (inelegantly)
...
class metrichor {
    ...
    loadUploadFiles() {
        ...
        if (!this._dirScanInProgress && remaining === 0) {
            ...
            const isFast5 = item => item.slice(-6) === '.fast5'
            let nonFast5 = (file, stats) => {
                let correctFormat = isFast5(file)
                let fileName = file.split('/').pop()
                let alreadyUploaded = this._uploadedFiles.indexOf(fileName) > -1
                let ignore = !correctFormat || alreadyUploaded
                // `file` is the absolute path to the file, and `stats` is an `fs.Stats` 
                // object returned from `fs.lstat()`. ƒuploadto
                return ignore
            }

            let ignorables = [`${this.uploadTo}/*`, "./downloads/*", nonFast5]
            readdir(this.config.options.inputFolder, ignorables, (err, files) => {

                // Rather than filename, we have an object:
                // { name: 'filename.fast5', batch: 'batch_name' }
                files = files.map(file => {
                    let full = file.replace(`${this.config.options.inputFolder}/`, '').split('/')
                    let name = full.pop()
                    let fileObject = { name }
                    if (full.length && full[0]) fileObject.batch = full[0]
                    return fileObject
                })

                this._dirScanInProgress = false;
                if (err) return this.log.error(`readdir error: ${err}`);
                this.log.info(`items in input folder: ${files.length}\tnew ${this.config.options.inputFormat} files: ${files.length}`);

                // each batch in has a size of uploadBatchSize
    ...
    uploadJob(item, completeCb) {
        ...
        this.uploadHandler(item, (errorMsg) => {
            ...
            this._stats.upload.enqueued = this._stats.upload.enqueued - 1;
            if (errorMsg) {
    ...
    // Look inside `folder` for a batch with a free slot
    // return it, or create a new batch and return that.
    findSuitableBatchIn(folder) {
        const targetBatchSize = 1000
        const prefix = 'batch_'
        const createBatch = () => mkdirp.sync(path.join(folder, `${prefix}${Date.now()}`))
        let batches = fs.readdirSync(folder).filter(d => d.slice(0, prefix.length) === prefix)
        if (!batches.length) return createBatch()
        let latestBatch = batches.pop()
        if (fs.readdirSync(latestBatch).length < targetBatchSize) return createBatch()
        return latestBatch
    }
    ...
    uploadHandler(item, completeCb) {
        ...
        let s3 = this.sessionedS3(),
            rs,
            fileId   = path.join(this.config.options.inputFolder, item.name),
            objectId = this.config.instance.bucketFolder + "/" + (this.config.instance.inputQueueName ? this.config.instance.inputQueueName + "/" : "") + item.name,
            timeoutHandle,
            completed = false;
    ...
    _moveUploadedFile(file, successCb) {
        ...
        mkdirp(this.uploadTo, (mkdirException) => {
