...
"use strict";
var request, AWS, queue, fs, path, os, mkdirp, proxy, options, _instance, _config;
...
metrichor.prototype = {
    ...
    attr : function (key, value) {
        ...
        if (options.hasOwnProperty(key)) {
            ...
            throw "Error: config object does not contain property " + key;
        }
        return this;
    },
    ...
    setOptions : function () {
        ...
        _config = {
            ...
            proxy                  : options.proxy,
            url                    : options.url || 'https://metrichor.com',

            /* below are the consolidated harness options for 0.7.0 */
            retention              : "on",
            telemetryCb            : options.telemetryCb,
            ...
            inFlightDelay          : options.inFlightDelay    || 600,           // wait 5 mins before resub
            waitTimeSeconds        : options.waitTimeSeconds  || 20,           // long-poll wait 20 seconds for messages
            downloadPoolSize       : options.downloadPoolSize || 10,           // MC-505 how many things to download at once
            filter                 : options.filter           || "on",
            downloadMode           : options.downloadMode     || "data+telemetry"
        };

        /* Container for workflow instance configuration. Needed? it's not even used... */
        _instance = {
            inputQueueName         : options.inputQueueName,
            inputQueueURL          : null,
            outputQueueName        : options.outputQueueName,
            outputQueueURL         : null,
            _discoverQueueCache    : {},
            _queueLengthTimeStamps : {},
            id_workflow_instance   : options.id_workflow_instance,
            bucket                 : options.bucket,
            bucketFolder           : options.bucketFolder,
    ...
    token : function (id, cb) { /* should this be passed a hint at what the token is for? */
        var that = this;
        return that._post('token', {id_workflow_instance: id || _instance.id_workflow_instance}, null, cb);
    },
    ...
    _get : function (uri, cb) {
        ...
        uri = "/" + uri + ".js?apikey=" + _config.apikey;
        srv = srv.replace(/\/+$/, ""); // clip trailing /s
        uri = uri.replace(/\/+/g, "/");

        if (_config.agent_version) {
            uri = uri + ";agent_version=" + _config.agent_version;
        }
    ...
    _post : function (uri, id, obj, cb) {
        ...
        if (_config.agent_version) {
            form.agent_version = _config.agent_version;
        }
    ...
    on: function (eventName, eventHandler) {
        ...
        if (eventName === "folderChange") {
            ...
            that._fsWatcher = fs.watch(_config.inputFolder, function (event, filename) {
        ...
        if (eventName === "downloadAvailable") {
            ...
            timedChecker = function () {
                ...
                that.sessionedSQS(function (sessionError, sqs) {
                    ...
                    if (_instance.outputQueueURL) {
                        return that.queueLength(_instance.outputQueueURL, workChecker, 1);
                    }

                    that.discoverQueue(sqs, _instance.outputQueueName,
                        function (queueURL) {
//                            that.log.info("discovered queue " + queueURL);
                            _instance.outputQueueURL = queueURL;
                            return that.queueLength(_instance.outputQueueURL, workChecker, 1);
                        },
    ...
    stop_everything: function (cb) {
        ...
        if (_instance.id_workflow_instance) {
            that.stop_workflow(_instance.id_workflow_instance, function () {
                that.log.info("workflow instance " + _instance.id_workflow_instance + " stopped");
            });
        }
    ...
    session: function (cb) {
        ...
        if (!that._stats.sts_expiration ||
            ...
            that.token(_instance.id_workflow_instance, function (tokenError, token) {
                if (tokenError) {
...
//                that.log.info(JSON.stringify(token));
                ...
                if (_config.proxy) {
                    AWS.config.update({
                        httpOptions: { agent: proxy(_config.proxy, true) }
                    });
                }

                AWS.config.update(_instance.awssettings);
    ...
    autoStart: function (id, cb) {
        ...
        that.start_workflow(id, function (workflowError, instance) {
             ...
             * chain
             */
            _instance.id_workflow_instance = instance.id_workflow_instance;
            _instance.chain               = instance.chain;
            _instance.remote_addr         = instance.remote_addr;
            _instance.bucket              = instance.bucket;
            _instance.inputQueueName      = instance.inputqueue;
            _instance.outputQueueName     = instance.outputqueue;
            _instance.awssettings.region  = instance.region;
            _instance.bucketFolder        = instance.outputqueue + "/" + instance.id_user + "/" + instance.id_workflow_instance;
    ...
    autoConfigure: function (instance, autoStartCb) {
        ...
        if (!that.uploadWorkerPool) {
            that.uploadWorkerPool = queue(_config.uploadPoolSize + 1);
        ...
        for (i = 1; i < _config.uploadPoolSize; i += 1) {
        ...
        fs.readdir(_config.inputFolder, function (readdirErr, files) {
            if (readdirErr) {
        ...
        if (!that.downloadWorkerPool) {
            that.downloadWorkerPool = queue(_config.downloadPoolSize + 1);
        ...
        mkdirp.sync(_config.outputFolder);
        telemetryLogPath = path.join(_config.outputFolder, "telemetry.log");
            ...
            that.discoverQueue(sqs, _instance.outputQueueName,
                function (queueURL) {
                    ...
                    try {
                        sqs.receiveMessage({
                            QueueUrl:            queueURL,
                            VisibilityTimeout:   _config.inFlightDelay,    // approximate time taken to pass/fail job before resubbing
                            MaxNumberOfMessages: _config.downloadPoolSize, // MC-505 - download multiple threads simultaneously
                            WaitTimeSeconds:     _config.waitTimeSeconds   // long-poll
...
//        that.log.info("telemetry:" + JSON.stringify(messageBody.telemetry));
        ...
        if (messageBody.telemetry) {
            ...
            if (_config.telemetryCb) {
                _config.telemetryCb(messageBody.telemetry);
            }
        }
        ...
        fn         = match ? match[1] : "";
        folder     = _config.outputFolder;

        if (_config.filter === 'on') {
        ...
        if (_config.downloadMode === "data+telemetry") {
    ...
    enqueueUploadJob: function (item, requeue) {
        ...
        that.uploadWorkerPool.defer(function (completeCb) {
            that.queueLength(_instance.inputQueueURL, function (len) {
    ...
    uploadHandler: function (item, successCb) {
        ...
        that.sessionedS3(function (sessionError, s3) {
            ...
            var rs,
                fileId   = path.join(_config.inputFolder, item),
                objectId = _instance.bucketFolder + "/" + (_instance.inputQueueName ? _instance.inputQueueName + "/" : "") + item;
    ...
    discoverQueue: function (sqs, queueName, successCb, failureCb) {
        ...
        if (_instance._discoverQueueCache[queueName]) {
            return successCb(_instance._discoverQueueCache[queueName]);
        }
        ...
        try {
            sqs.getQueueUrl({ QueueName: queueName }, function (getQueueErr, getQueue) {
                if (getQueueErr) {
                    if (_config.proxy && (String(getQueueErr)).match(/Unexpected close tag/)) {
                ...
                _instance._discoverQueueCache[queueName] = getQueue.QueueUrl;
    ...
    uploadComplete: function (objectId, item, successCb) {
        ...
        that.sessionedSQS(function (sessionError, sqs) {
            ...
            if (_instance.inputQueueURL) {
                return that.sendMessage(sqs, objectId, item, successCb);
            }

            that.discoverQueue(sqs, _instance.inputQueueName,
                function (queueURL) {
                    _instance.inputQueueURL = queueURL;
                    return that.sendMessage(sqs, objectId, item, successCb);
                },
                function (err) {
                    that.enqueueUploadJob(item, 1);
    ...
    sendMessage: function (sqs, objectId, item, successCb) {
        var that    = this,
            message = {
                bucket:               _instance.bucket,
                outputQueue:          _instance.outputQueueName,
                remote_addr:          _instance.remote_addr,
                apikey:               _config.apikey,
                id_workflow_instance: _instance.id_workflow_instance,
...
//                message.inputFolder = that.runFolder; // MC-960 folder aggregation messages
        ...
        if (_instance.chain) {
            try {
                message.components        = JSON.parse(JSON.stringify(_instance.chain.components)); // low-frills object clone
                message.targetComponentId = _instance.chain.targetComponentId; // first component to run
...
//        that.log.info("sending message " + JSON.stringify(message));
        ...
        try {
            sqs.sendMessage({
                QueueUrl:    _instance.inputQueueURL,
                MessageBody: JSON.stringify(message)

            }, function (sendMessageError) {
                var fileFrom = path.join(_config.inputFolder, item),
                    folderTo = path.join(_config.inputFolder, "uploaded"),
...
//                that.log.info("message sent " + JSON.stringify(message));
    ...
    queueLength: function (queueURL, cb, forceUpdate) {
        ...
        if (queueURL &&
                (forceUpdate ||
                    (!_instance._queueLengthTimeStamps[queueURL] ||
                     _instance._queueLengthTimeStamps[queueURL] < now))) {

            _instance._queueLengthTimeStamps[queueURL] = now;
            _instance._queueLengthTimeStamps[queueURL].setMinutes(_instance._queueLengthTimeStamps[queueURL].getMinutes() + 1);
        } else {
            return cb();
        }
...
//                        that.log.info("found queue length of " + queueURL + " = " + len); // + " querying again no sooner than " + _instance._queueLengthTimeStamps[queueURL]);
                        return cb(len);
                    }
                });
