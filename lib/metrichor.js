...
class metrichor {
    ...
    downloadAvailable(cb) {
        ...
        this.discoverQueue(sqs, this.config.instance.outputQueueName,
            (queueURL) => {
                ...
                try {
                    sqs.receiveMessage({
                        AttributeNames: "ApproximateReceiveCount", // to check if the same message is received multiple times
                        QueueUrl:            queueURL,
    ...
    processMessage(message, completeCb) {
        ...
        this.log.info("download.processMessage" + message.MessageId + " . count " + message.attributes);
