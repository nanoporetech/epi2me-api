        ...
        if (eventName === "downloadAvailable") {
            ...
            workChecker = function (len) {
                ...
                var workClosure = function () {
                    that.log("unflagging running downloader");
            ...
            timedChecker = function () {
                if (that.downloaderRunning) {
                    that.log("download already running " + that.downloaderRunning);
                    return;
                }
    ...
    session: function (cb) {
        ...
        if (!that._stats.sts_expiration ||
            ...
            that.token(that._idWorkflowInstance, function (tokenError, token) {
                if (tokenError) {
                    that.log("failed to fetch session token: " + tokenError.error ? tokenError.error : tokenError);
        ...
        if (that.downloadMode === "data+telemetry") {
            ...
            that.sessionedS3(function (sessionError, s3) {
                ...
                file.on("close", function (writeStreamError) {
                    ...
                    that.log("downloaded " + messageBody.path);
...
//                message.inputFolder = that.runFolder; // MC-960 folder aggregation messages
        ...
        if (that.chain) {
            try {
                ...
                that.log("exception parsing components JSON " + String(jsonException));
                that.enqueueUploadJob(item, 1);
...
//        that.log("sending message " + JSON.stringify(message));
...
//                        that.log("found queue length of " + queueURL + " = " + len); // + " querying again no sooner than " + that._queueLengthTimeStamps[queueURL]);
                        return cb(len);
                    }
                });
