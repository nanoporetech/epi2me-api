...
"use strict";
var request, AWS, queue, fs, path, os, mkdirp, proxy, readdirp, es, _config;
...
function metrichor(opt_string) {
    ...
    _config = {
        options: {
            ...
            uploadQueueLimit       : opts.uploadQueueLimit || 500,          // Maximum number of files in the queue at once
            fileCheckInterval      : 10,                                    // Seconds between loadUploadFiles()
...
metrichor.prototype = {
    ...
    on: function (eventName, eventHandler) {
        ...
        if (eventName === "newFile") {
            ...
            if (that._readdirpStream) {
                that._readdirpStream.destroy();
            }
...
//                that.log.info(JSON.stringify(token));
    ...
    autoConfigure: function (instance, autoStartCb) {
        ...
        that._fileCheckInterval = setInterval(function fileCheckInterval() {
            ...
            that.loadUploadFiles.apply(that);
    ...
    loadUploadFiles: function () {
        ...
        /**
         * MC-1836 - Streams implementation of readdir
         * enqueue files from the inputFolder until the upload queue exceeds the uploadQueueLimit
         */

        var that = this,
            queueLength = that._stats.upload.queueLength,
            fileExp = /\.fast5$/,   // only .fast5 files
            dirExp = /[\\\/]/;      // ignore sub-directories

        if (queueLength >= _config.options.uploadQueueLimit) {
            return;
        }

        function destroy(msg) {
            that.log.info('destroying readdir stream. ' + (msg || ''));
            if (that._readdirpStream && that._readdirpStream.destroy) {
                that._readdirpStream.destroy();
            }
            that._readdirpStream = null;
        }
        ...
        that._readdirpStream = readdirp(
            {
                root: _config.options.inputFolder,
                fileFilter: function (d) {
                    return !d.path.match(dirExp) && d.path.match(fileExp);
                }
            }
        );

        /*jslint node: true, stupid: true */
        that._readdirpStream
            .on('warn', function (err) {
                that.log.warn('Non-fatal stream error: ' + err);
            })
            .on('error', function (err) {
                that.log.error('Fatal stream error: ' + err);
                destroy();
            })
            ...
            .on('data', function (entry) {
                if (queueLength >= _config.options.uploadQueueLimit) {
                    destroy('uploadQueueLimit reached');
                } else if (entry) {
                    // Prevent loading same file more than once
                    if ((that._seenFiles && that._seenFiles[entry.path]) || entry.path.match(/uploaded/) || entry.path.match(/downloads/)) {
                        return;
                    }
                    that._seenFiles[entry.path] = {
                        size: entry.size
                    };
                    queueLength += 1;
                    that.enqueueUploadJob(entry.path);
                }
            });
    },
            ...
            try {
                that.uploadHandler(item, function (result) {
                    if (isNaN(result)) {
                        ...
                        if (that._seenFiles[item]) {
                            that._seenFiles[item].state = 'uploaded';
                        }
