...
class metrichor {
    ...
    loadUploadFiles() {
        ...
        this.log.info(`loadUploadFiles: ${remaining} batches in the inputBatchQueue`);
    ...
    enqueueUploadFiles(files) {
        ...
        if (this.config.options.filetype === '.fastq') {
            this.inputBatchQueue.defer(batch_complete => {
                ...
                let statQ = queue(1);
                this.log.info(`enqueueUploadFiles.countFileReads: counting FASTQ reads per file`);
                files.forEach(file => {
                ...
                statQ.awaitAll(() => {
                    this.log.info(`enqueueUploadFiles.enqueued: ${this._stats.upload.enqueued}`);
                ...
                try {
                    ...
                    readStream
                        .on('close', () => {
                            ...
                            statFile();
                            this.log.info(`${file.id}: upload and mv done`);
