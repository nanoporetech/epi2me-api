...
class metrichor {
    ...
    enqueueUploadFiles(files) {
        let maxFiles, maxFileSize = 0, settings = {};
    ...
    workflow(id, obj, cb) {
        if (!cb) {
            ...
            this._read("workflow", id, (err, details) => {
                ...
                if (toFetch && toFetch.length > 0) {
                    const promises = toFetch.map(param => {
                        return new Promise((resolve, reject) => {

                            const uri = param.values.source
                                .replace("{{EPI2ME_HOST}}", "")
                                .replace("{{EPI2ME_API_KEY}}", this.config.options.apikey);
    ...
    bundle_workflow(id_workflow, filepath, cb, progressCb) {
        ...
        new Promise(
            (resolve) => {
                utils._pipe(`workflow/bundle/${id_workflow}.tar.gz`,
                    filepath,
                    this.config.options,
                    resolve, // todo: add error check!
                    progressCb);
            })
            .then(
                (resolve) => {
                    utils._read("workflow", id_workflow, (err, data) => {
                        // 2018-09-10 (rmp): portal does not serve a
                        // complete workflow.json in bundle_workflow (it's
                        // missing parameters for example), so we quietly
                        // refetch and overwrite here
                        let WORKFLOW_DIR = path.join(this.config.options.url, "workflows");
                        let wf_path = path.join(WORKFLOW_DIR, id_workflow, "workflow.json");
                        let wf_json_stream = fs.createWriteStream(wf_path);
                        wf_json_stream.write(JSON.stringify(data));
                        wf_json_stream.end();
                        resolve();
                    });
                })
