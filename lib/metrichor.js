...
metrichor.prototype = {
    ...
    workflow_config : function (id, cb) {
        return this._get('workflow/' + id + '/config', cb);
    },
    ...
    autoConfigure: function (instance, autoStartCb) {
        ...
        _config.instance.param_config         = instance.userDetails; // MC-2387 - parameterisation
            ...
            fs.readdir(_config.options.inputFolder, function (err, files) {
                ...
                files = files
                    .map(function (v) {
                        stat = null;
                        try {
                            stat = fs.statSync(path.join(_config.options.inputFolder, v)).mtime.getTime();
                        } catch (ignore) {
                        }
    ...
    _initiateDownloadStream: function (s3, messageBody, message, outputFile, completeCb) {
        ...
        function deleteFile() {
            try {
                ...
                that.log.warn("failed to remove file: " + outputFile + ' ' + String(unlinkException));
            }
        }
        ...
        file.on("finish", function () {
            ...
            .on("error", function (writeStreamError) {
                that.log.error("error in download write stream " + writeStreamError);
                try {
                    onStreamError();
                } catch (e) {
                    that.log.error("error handling download writeStreamError: " + e);
                }
            });
    ...
    sendMessage: function (sqs, objectId, item, successCb) {
        var that    = this,
            message = {
                ...
                param_config:         _config.instance.param_config, // MC-2397 - bind param_config to each sqs message
                apikey:               _config.options.apikey,
...
//        that.log.info("sending message " + JSON.stringify(message));
        ...
        try {
            sqs.sendMessage({
                ...
                fileFrom = path.join(_config.options.inputFolder, item);
...
try {
    var readStream = fs.createReadStream(fileFrom);
    var writeStream = fs.createWriteStream(fileTo);

    writeStream.on("error", function (writeStreamError) {
        that.log.error("uploaded file: error in writeStream. " + writeStreamError);
    });
    ...
    readStream
        ...
        .on("error", function (readStreamError) {
            that.log.error("uploaded file: error in readStream. " + readStreamError);
            successCb("failed to rename uploaded file");
        });

} catch (renameStreamException) {
    that.log.warn("failed to move uploaded file into upload folder: " + String(renameStreamException));
}
                });
            });
