...
"use strict";
var request, AWS, queue, fs, path, os, mkdirp, proxy, _config, underscore;
...
function metrichor(opt_string) {
    ...
    _config = {
        options: {
            ...
            uploadPoolSize         : 3,                                     // Parallelism of upload queue
...
metrichor.prototype = {
    ...
    stats: function (key) {
        if (this._stats[key]) {
            ...
            if (key === "upload") {
                this._stats.upload.total = this._inputFiles.length;
            }
        }
        ...
        request.get(
            {
                ...
                headers: {
                    'X-Metrichor-Client': _config.options.user_agent
                }
            },
            function (e, r, body) {
    ...
    autoConfigure: function (instance, autoStartCb) {
        ...
        that._inputFiles = []; // container for files that have been successfully uploaded, but failed to move
    ...
    _statInputFiles: function (files, parallelism) {

        var that = this;

        return new Promise(function (resolve, reject) {
            // Filter is much faster than a for loop or forEach
            var fsStatQueue = queue(parallelism || 20); // run 20 fs.stat in parallel

            files.forEach(function (fn) {
                fsStatQueue.defer(function (cb) {
                    try {
                        fs.stat(path.join(_config.options.inputFolder, fn), function (err, stat) {
                            if (err) {
                                cb(err);
                            } else {
                                cb(null, {
            ...
            fsStatQueue.awaitAll(function (err, files) {
                if (!err && files) {
                    that.log.info('fs.stat all input files: success');
                    resolve(files);
                } else {
                    that.log.error(err);
                    reject(err);
    ...
    loadUploadFiles: function () {
        ...
        var that = this, remaining, files_left;

        if (!underscore.isArray(that._inputFiles)) {
            that._inputFiles = [];
        }

        remaining = that.uploadWorkerPool ? that.uploadWorkerPool.remaining() : 0;
         // only consider files with the specified extension
        files_left = underscore.filter(that._inputFiles, {in_flight: false, uploaded: false}).length;

        that.log.info('loadUploadFiles: uploadWorkerPool remaining: ' + remaining + '. files waiting: ' + files_left);

        function unstashFiles() {
            that.log.info('pushing files to enqueueUploadJob');
            underscore.chain(that._inputFiles)
                .filter({in_flight: false, uploaded: false})
                .first(_config.options.uploadQueueThreshold)
                .forEach(that.enqueueUploadJob.bind(that));
        }

        if (!that._dirScanInProgress && (files_left < _config.options.uploadQueueThreshold)) {
            ...
            fs.readdir(_config.options.inputFolder, function (err, files) {
                ...
                that.log.info('found ' + files.length + ' items in the input folder');
                var filtered = files.filter(function (fn) {
                    // file extension is of right format
                    // _inputFiles does not already contain the file
                    return path.extname(fn) === _config.options.inputFormat && !underscore.findWhere(that._inputFiles, { file: fn });
                });
                that.log.info('preparing ' + filtered.length + ' new ' + _config.options.inputFormat + ' files for upload');
                if (filtered.length) {
                    that.log.info('fs.stat all input files');
                    that._statInputFiles(filtered)
                        .then(function (file_list) {
                            file_list.forEach(function (d) {
                                that._inputFiles.push(d);
                            });
                            unstashFiles();
                        });
                }
    ...
    timedEnqueueUploadJob: function (item) {
        ...
        that.uploadWorkerPool.defer(function (completeCb) {
            ...
            if (item.in_flight || item.uploaded || item.error > 5) {
                return completeCb();
            }
            ...
            timeoutHandle = setTimeout(function () {
                ...
                that.log.error("Error: uploadWorkerPool timeoutHandle. Clearing queue slot for file: " + item.file + " file_size: " + item.size);
    ...
    uploadHandler: function (item, successCb) {
        ...
        var rs,
            fileId   = path.join(_config.options.inputFolder, item.file),
            objectId = _config.instance.bucketFolder + "/" + (_config.instance.inputQueueName ? _config.instance.inputQueueName + "/" : "") + item.file;
                ...
                try {
                    s3.upload(params, options, function (uploadStreamErr) {
                        if (uploadStreamErr) {
                            ...
                            return successCb("uploadStreamError " + String(uploadStreamErr)); // close the queue job
                        }
                    ...
                    successCb("failed to upload: " + String(uploadStreamException)); // close the queue job
                    destroy();
                }
            });
    ...
    uploadComplete: function (objectId, item, successCb) {
        var that = this,
            sqs = that.sessionedSQS();
        that.log.info("uploaded " + item.file + " to " + objectId);
...
//                message.inputFolder = that.runFolder; // MC-960 folder aggregation messages
    ...
    _moveUploadedFile: function (item, successCb) {
        ...
        fileFrom = path.join(_config.options.inputFolder, item.file);
        fileTo = path.join(folderTo, item.file);
        ...
        function onSuccess() {
            that._stats.upload.totalSize += item.size; // total size of uploaded data
            item.uploaded = true;
        }
        ...
        function destroyReadStream() {
            try {
                if (readStream.destroy) {
                    that.log.error("destroying upload readstream for " + item.file);
                    readStream.destroy();
                }
        ...
        function onError(err) {
            if (!streamErrorFlag) {
                streamErrorFlag = true; // prevent from firing twice
                onSuccess(); // flag it as successfully uploaded. file will remain in input folder
        ...
        mkdirp(folderTo, function (mkdirException) {
            if (mkdirException && !String(mkdirException).match(/EEXIST/)) {
                onError("mkdirpException " + String(mkdirException));
            } else {
