...
metrichor.prototype = {
    ...
    stats: function (key) {
        if (this._stats[key]) {
            ...
            if (key === "upload") {
                this._stats.upload.total = underscore.map(this._inputFiles).length;
            }
        }
    ...
    loadUploadFiles: function () {
        ...
        that.log.info('loadUploadFiles:: uploadWorkerPool remaining: ' + remaining + ' - _inputFiles.length: ' + files_left);
        ...
        } else if (files_left < _config.options.uploadQueueThreshold) {
            unstashFiles();
        }
    },
