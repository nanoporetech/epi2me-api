...
class metrichor {
	...
	outputFile.replace("/pass/","/PASS/"); // MC-5612: uppercase default "pass" folder
            ...
            this._read("workflow", id, (err, details) => {
                ...
                if (toFetch && toFetch.length > 0) {
                    ...
                    Promise.all(promises)
                        ...
                        .catch((err) => { this.log.error(`${id}: error fetching config and parameters (${err})`); callback;});
                } else {
                    Promise.all([configPromise]).then(() => { callback(null, details); }).catch((err) => { this.log.error(`${id}: error fetching config and parameters (${err})`); callback;});
                }
            });
            return;
        }
