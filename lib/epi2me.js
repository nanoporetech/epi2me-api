...
import fs       from "fs-extra"; /* MC-565 handle EMFILE gracefully; use Promises */
import os       from "os";
...
export default class EPI2ME {
    ...
    async sessionedSQS() {
        ...
        new Promise((resolve) => {
            ...
            .then(() => {
                return Promise.resolve(new AWS.SQS());
            })
            .catch((err) => {
                this.log.error(err);
            });
    ...
    _moveSkippedFile(file, successCb) {
        ...
        fs.mkdirp(path.join(this.skipTo, fileBatch), (mkdirException) => {
            if (mkdirException && !String(mkdirException).match(/EEXIST/)) {
    ...
    _moveUploadedFile(file, successCb) {
        ...
        fs.mkdirp(path.join(this.uploadTo, fileBatch), (mkdirException) => {
            if (mkdirException && !String(mkdirException).match(/EEXIST/)) {
