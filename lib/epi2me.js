...
export default class EPI2ME {
    ...
    async loadAvailableDownloadMessages() {
        ...
        if (!this.config.instance.outputQueueURL) {
            ...
            await this.discoverQueue(sqs, this.config.instance.outputQueueName)
                .then((queueURL) => {
    ...
    deleteMessage(message) {
        ...
        return this.discoverQueue(sqs, this.config.instance.outputQueueName)
            .then((queueURL) => {
                sqs.deleteMessage({
                    QueueUrl:      queueURL,
            ...
            .catch((reason) => {
                this.log.error(`deleteMessage exception: ${String(reason)}`);
                this._stats.download.failure[reason] = this._stats.download.failure[reason] ? this._stats.download.failure[reason] + 1 : 1;
            });
    }
    ...
    discoverQueue(sqs, queueName) {
        return new Promise((resolve, reject) => {

            if (this.config.instance._discoverQueueCache[queueName]) {
                return resolve(this.config.instance._discoverQueueCache[queueName]);
            }
