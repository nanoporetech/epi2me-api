...
export default class EPI2ME {
    ...
    async session(cb) {
        ...
        if (!this._stats.sts_expiration ||
            ...
            && !this.sessionQueue.busy)) {               /* Throttle to n=1: bail out if there's already a job queued */
            ...
            await this.sessionQueue
                .then(() => {
                    this.sessionQueue.busy = true;
                })
                .then(async () => {
                    await this.fetchInstanceToken(e => {
                        if (e) {
                            this.log.error(e);
                            err = e;
                        }
                ...
                .catch((e) => {
                    this.log.debug(`session error ${String(e)}`);
                    this.sessionQueue.busy = false;
                    err = e;
                });
        }
    ...
    async loadAvailableDownloadMessages() {
        if (!this.queueLengthQueue) {
            this.queueLengthQueue = Promise.resolve();
            this.queueLengthQueue.busy = false;
        }

        if (this.queueLengthQueue.busy) {
            /* don't build up a backlog by queuing another job */
            return;
        }

        this.queueLengthQueue.busy = true;

        if (!this.config.instance.outputQueueURL) {
            let sqs = this.sessionedSQS();
            try {
                let queueURL = await this.discoverQueue(sqs, this.config.instance.outputQueueName);
        ...
        await this.queueLengthQueue
            .then(() => {
                let p = new Promise((resolve) => {
                    this.queueLength(this.config.instance.outputQueueURL, len => this.downloadWork(len, resolve));
                });
                return p;
            })
            .then(() => {
    ...
    async downloadAvailable(cb) {
        ...
        try {
            ...
            try {
                let receiveMessageSet = await sqs.receiveMessage({
                ...
                this.receiveMessages(null, receiveMessageSet, cb);

            } catch(receiveMessageErr) {
                this.log.error("receiveMessage exception: " + String(receiveMessageErr));
                this.receiveMessages(receiveMessageErr, null, cb);
            }

        } catch (discoverQueueErr) {
            this._stats.download.failure[discoverQueueErr] = this._stats.download.failure[discoverQueueErr] ? this._stats.download.failure[discoverQueueErr] + 1 : 1;
            return cb();
        }
    }
    ...
    enqueueUploadFiles(files) {
            ...
            this.inputBatchQueue
                .push(async () => {
                    ...
                    files.forEach(item => {
                        ...
                        uploadWorkerPool.push(new Promise((resolve, reject) => {
                            this.uploadJob(item, err => {
                                return err ? resolve() : reject(err);
                            });
                        }));
                    });
                    ...
                    await Promise
                        .all(uploadWorkerPool)
                        .catch(err => {
                            this.log.error(`uploadWorkerPool (non-fastq) exception ${String(err)}`);
                        });
                });
        }
        ...
        return Promise
            .all(this.inputBatchQueue)
            .then(() => {
                this.log.info("inputBatchQueue slot released. trigger loadUploadFiles");
                setImmediate(this.loadUploadFiles); // immediately load more files
                return;
            })
            .catch(err => {
    ...
    async discoverQueue(sqs, queueName) {
    ...
    async uploadComplete(objectId, file, successCb) {
        let sqs = this.sessionedSQS();
        ...
        try {
            let queueURL = await this.discoverQueue(sqs, this.config.instance.inputQueueName);
    ...
    async sendMessage(sqs, objectId, file, successCb) {
