...
AWS = (function(superClass) {
  ...
  function AWS(options1, api, ssd) {
    ...
    this.getSQSCount = bind(this.getSQSCount, this);
  ...
  AWS.prototype.start = function(instance, done) {
      ...
      return function(error, aws) {
        return aws.sqs.getQueueUrl({
          ...
          return aws.sqs.getQueueUrl({
            ...
            return _this.getSQSCount(function(error) {
              _this.SQSmonitor = setInterval(_this.getSQSCount, 5000);
              if (error) {
  ...
  AWS.prototype.getSQSCount = function(done) {
    var count, getCountForQueue;
    count = {};
    getCountForQueue = (function(_this) {
      return function(queue, done) {
        return _this.token(function(error, aws) {
          ...
          return aws.sqs.getQueueAttributes(options, function(error, attr) {
    ...
    return getCountForQueue('input', (function(_this) {
      return function(error, input) {
        return getCountForQueue('output', function(error, output) {
          var sqsCount;
          sqsCount = {
            input: input,
            output: output
          };
          if (diff(_this.stats.sqs, sqsCount)) {
            _this.stats.sqs = sqsCount;
          }
      ...
      return function(error, aws) {
        ...
        return _this.ssd.getFile(file.source, function(error, data) {
          ...
          S3Object = {
            ...
            Key: [_this.instance.outputqueue, _this.instance.id_user, _this.instance.id_workflow_instance, _this.instance.inputqueue, file.name].join('/'),
            Body: data
          };
  ...
  AWS.prototype.downloadFile = function(sqsMessage, done) {
    ...
    return this.token((function(_this) {
      return function(error, aws) {
        var filename, folder, messageBody, mode, stream, streamOptions;
        if (error) {
          return done(error);
        }
        ...
        if (messageBody.telemetry) {
          _this.ssd.appendToTelemetry(messageBody.telemetry);
        }
        ...
        return _this.ssd.saveDownloadedFile(stream, path.join(folder, filename), function(error) {
          var deleteOptions;
          if (error) {
