var EventEmitter, SSD, WatchJS, async, chokidar, completeBatch, disk, fast5, fs, isBatch, isPartial, isProcessing, mkdirp, mv, os, partialBatch, path, pathRoot,
...
SSD = (function(superClass) {
  ...
  SSD.prototype.checkPermissions = function(done) {
    return fs.access(this.inputFolder, fs.R_OK, function(error) {
      if (error) {
        return done(error);
      }
      return fs.access(this.outputFolder, fs.W_OK, function(error) {
        if (error) {
          return done(error);
        }
        return done();
