var EventEmitter, SSD, WatchJS, async, chokidar, completeBatch, fast5, fs, isBatch, isPartial, isProcessing, mkdirp, mv, os, partialBatch, path,
...
SSD = (function(superClass) {
  ...
  SSD.prototype.saveDownloadedFile = function(stream, filename, telemetry, done) {
    var destination, folder, localFile, preExisting, ref;
    if (telemetry.hints) {
      folder = telemetry.hints.folder || '';
      if (this.options.filterByChannel === 'on' && telemetry.hints.channel_id) {
        folder = path.join(folder, telemetry.hints.channel_id);
      }
    } else {
      if ((ref = telemetry.json) != null ? ref.exit_status : void 0) {
    ...
    destination = path.join(this.options.outputFolder, folder, filename);
