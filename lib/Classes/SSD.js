var EventEmitter, SSD, WatchJS, async, chokidar, completeBatch, disk, fast5, fs, isBatch, isPartial, isProcessing, mkdirp, mv, os, partialBatch, path, pathRoot, split,
...
SSD = (function(superClass) {
        ...
        if (!(batches != null ? batches.length : void 0)) {
          return fs.readdir(_this.options.inputFolder, function(error, files) {
            if (!files.filter(fast5).length) {
              _this.emit('status', "No files to upload");
  ...
  SSD.prototype.saveDownloadedFile = function(stream, filename, telemetry, done) {
    var destination, failed, folder, localFile, ref, ref1, saveFailed, successful, timeout;
  ...
  SSD.prototype.freeSpace = function(done) {
    ...
    return disk.check(pathRoot(this.options.outputFolder), function(error, total, free, status) {
      var megabytes_free;
      if (error) {
        return done(error);
      }
  ...
  countLinesInFile = function(filePath, done) {
    var lineCount, readError;
    lineCount = 0;
    readError = false;
    return fs.createReadStream(filePath).pipe(split()).on('data', function(line) {
      return lineCount++;
    }).on('end', function() {
      if (readError) {
        return;
