...
class LocalDirectory
  constructor: (@options) ->
    ...
    @subdirectories = ['pending', 'uploaded', 'upload_failed', 'downloads']
  ...
  reset: (done) ->
    @stop =>
      rootWalker = fs.walk @inputFolder
      rootWalker.on "file", (root, stat, next) =>
        source = path.join root, stat.name
        destination = "#{@inputFolder}/#{stat.name}"
        mv source, destination, next
      rootWalker.on 'end', =>
        pendingWalker = fs.walk "#{@inputFolder}/pending"
  ...
  start: (@instance, done) ->
    mkdirp "#{@inputFolder}/#{subdirectory}" for subdirectory in @subdirectories
    @createBatches yes, =>
      @uploadScan()
      @watcher = chokidar.watch @inputFolder, { depth: 0, ignoreInitial: yes }
      @watcher.on 'add', (path) =>
        @watcher.unwatch @inputFolder
        @createBatches yes, =>
          @watcher.add @inputFolder
    ...
    fs.readdir @inputFolder, (error, files) =>
      ...
      createBatch = (batch, next) =>
        batch = batch.map (file) => return file =
          source: "#{@inputFolder}/#{file}"
          destination: "#{@inputFolder}/pending/batch_#{Date.now()}/#{file}"
  ...
  testFile: ->
    fs.writeFile "#{@inputFolder}/test_#{Date.now()}.fast5", 'test', (error) =>
  ...
  uploadScan: ->
    @scannerKilled = no
    fs.readdir "#{@inputFolder}/pending", (error, batches) =>
      ...
      if not batches?.length
        return fs.readdir @inputFolder, (error, files) =>
          files = files.filter (item) -> return item.slice(-6) is '.fast5'
      ...
      source = "#{@inputFolder}/pending/#{batches[0]}"
  ...
  uploadComplete: (batch) ->
    source = "#{@inputFolder}/pending/#{batch}.processing"
    destination = "#{@inputFolder}/uploaded"
  ...
  stop: (done) ->
    @killUploadScan()
    @watcher.unwatch(@inputFolder).close() if @watcher
    ...
    fs.readdir "#{@inputFolder}/pending", (error, batches) =>
      ...
      batch = batches[0]
      source = "#{@inputFolder}/pending/#{batch.slice(0, -11)}"
